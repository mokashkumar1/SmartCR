...
export default function TakingPage() {
  ...
  const [loading, setLoading] = useState(true), [saving, setSaving] = useState(false), [online, setOnline] = useState(navigator.onLine), [lastAction, setLastAction] = useState(null), [synced, setSynced] = useState(false), [reviewing, setReviewing] = useState(false)
  ...
  const updateStatus = (studentId, status) => {
    const map = { ...statusMap, [studentId]: status }
    setStatusMap(map); persist(map); setSynced(false)
  }
  ...
  if (reviewing) return <div className="min-h-[100dvh] bg-surface-bg pb-6"><PageHeader title="Review attendance" backTo="/classes"/><main className="mx-auto max-w-lg px-4 py-5"><div className="mb-4 flex items-center justify-between text-sm"><span className="text-status-success font-semibold">{present} present</span><span className="text-status-error font-semibold">{absent} absent</span></div><p className="mb-4 text-sm text-dark-60">Tap a status to correct any mistake before saving.</p><div className="space-y-2">{students.map(student => <div key={student.id} className="premium-card flex items-center gap-3 p-3"><div className="min-w-0 flex-1"><p className="font-bold text-dark">{student.roll_number}</p><p className="truncate text-sm text-dark-60">{student.name}</p></div><div className="flex gap-1"><button onClick={() => updateStatus(student.id, 'absent')} aria-label={`Mark ${student.name} absent`} className={`h-11 min-w-11 rounded-lg px-3 text-xs font-bold ${statusMap[student.id] === 'absent' ? 'bg-status-error text-white' : 'bg-status-error-light text-status-error'}`}>A</button><button onClick={() => updateStatus(student.id, 'present')} aria-label={`Mark ${student.name} present`} className={`h-11 min-w-11 rounded-lg px-3 text-xs font-bold ${statusMap[student.id] === 'present' ? 'bg-status-success text-white' : 'bg-status-success-light text-status-success'}`}>P</button></div></div>)}</div><div className="sticky bottom-0 -mx-4 mt-5 border-t border-border bg-surface-bg/[.97] p-4 backdrop-blur"><Button size="giant" className="w-full" onClick={finish} disabled={saving}>{saving ? 'Saving…' : 'Save & view report'}</Button></div></main></div>
  if (!current) return <div className="min-h-[100dvh] bg-surface-bg"><PageHeader title="Ready to finish" backTo="/classes" /><main className="max-w-md mx-auto px-4 pt-8"><section className="premium-card p-6 text-center"><div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-full bg-status-success-light text-status-success"><Check size={28}/></div><p className="subtle-label">Attendance complete</p><h1 className="mt-2 text-2xl font-bold text-dark">{students.length} students marked</h1><div className="mt-6 grid grid-cols-3 divide-x divide-border rounded-xl bg-surface-muted py-3"><div><b className="text-lg text-status-success">{present}</b><p className="text-xs text-dark-60">Present</p></div><div><b className="text-lg text-status-error">{absent}</b><p className="text-xs text-dark-60">Absent</p></div><div><b className="text-lg text-dark">{students.length ? Math.round(present / students.length * 100) : 0}%</b><p className="text-xs text-dark-60">Rate</p></div></div><Button size="giant" className="mt-6 w-full" onClick={() => setReviewing(true)}>Review & edit</Button><Button variant="neutral" size="lg" className="mt-3 w-full" onClick={finish} disabled={saving}>{saving ? 'Saving…' : 'Save without review'}</Button></section></main></div>
  return <div className="attendance-screen bg-surface-bg"><PageHeader title="Take attendance" backTo="/classes"/><main className="attendance-main mx-auto w-full max-w-lg px-4 pt-5"><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><ProgressBar current={marked} total={students.length}/></div><button onClick={undo} disabled={!lastAction} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-surface-muted text-dark disabled:opacity-35" aria-label="Undo last mark"><Undo2 size={19}/></button></div><div className="mt-3 flex justify-between text-xs font-medium"><span className="text-status-success">{present} present</span><span className="text-status-error">{absent} absent</span><span className="text-dark-60">{students.length-marked} left</span></div><AnimatePresence mode="wait"><motion.article key={current.id} drag="x" dragConstraints={{ left: 0, right: 0 }} onDragEnd={(_, info) => info.offset.x > 90 ? mark('present') : info.offset.x < -90 ? mark('absent') : undefined} initial={{opacity:0,y:10,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:-8,scale:.98}} transition={{duration:.18}} className="attendance-card premium-card mt-6 min-h-[230px] p-6 flex flex-col justify-center text-center select-none"><p className="subtle-label">Roll number</p><div className="mt-2 break-words text-[clamp(2rem,9vw,3.25rem)] font-bold tracking-[-.04em] text-dark">{current.roll_number}</div><div className="attendance-divider mx-auto my-5 h-px w-12 bg-border"/><p className="text-[clamp(1.15rem,5vw,1.5rem)] font-semibold leading-snug text-dark break-words">{current.name}</p><p className="attendance-hint mt-4 text-xs text-dark-60">Swipe or use the buttons below</p></motion.article></AnimatePresence><div className="mt-4 flex justify-center"><span className="inline-flex items-center gap-1.5 text-xs text-dark-60">{online ? <Cloud size={14}/> : <WifiOff size={14}/>}{online ? (synced ? 'Synced' : 'Saving in background…') : 'Offline · saved on this device'}</span></div></main><div className="attendance-actions border-t border-border bg-surface-card/[.97] px-4 pt-3 pb-[calc(12px+env(safe-area-inset-bottom))] backdrop-blur-xl"><div className="mx-auto grid max-w-lg grid-cols-2 gap-3"><Button variant="danger" size="giant" onClick={() => mark('absent')} className="h-16 rounded-2xl text-base"><X size={22} className="mr-2"/>Absent</Button><Button variant="success" size="giant" onClick={() => mark('present')} className="h-16 rounded-2xl text-base"><Check size={22} className="mr-2"/>Present</Button></div></div></div>
}
